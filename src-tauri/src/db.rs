...
pub fn generate_recurring(conn: &Connection, year: i32, month: u32) -> Result<()> {
    // Modificado para gerar até o fim do ano (Dezembro) em vez de parar no mês atual
    // Isso garante que despesas, receitas e investimentos sejam salvos para "todos os meses" do ano
    let target_date = NaiveDate::from_ymd_opt(year, 12, 31).unwrap();
    println!(">>> Iniciando processamento de recorrências até 12/{}", year);
